...
@Path(DoNotMergeStoreIdService.SERVER_PATH)
public class DoNotMergeStoreIdService implements AdvertisableService
{
    private NeoServer neoServer;
    public static final String SERVER_PATH = "server/storeid";

    public DoNotMergeStoreIdService( @Context NeoServer neoServer )
    {
    ...
    public Response getVersionAndEditionData()
    {
        return Response.ok( "{\"version\": \"" + neoDatabaseVersion( neoServer ) + "\", " +
                ...
                "\"storeid\": \"" + neoStoreId( neoServer ) + "\"" +
                "}",
                APPLICATION_JSON )
                .build();
    }

    private String neoStoreId( NeoServer neoServer )
    {
      return Integer.toHexString( neoServer.getDatabase().getGraph().storeId().hashCode() );
    }
